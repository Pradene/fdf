...
static int	get_offy(t_data *data)
{
	return (data->offy + HEIGHT / 2
		- sin(rad(data->angleY))
		* (data->map.width * data->distance * data->scale / 2 / 2)
		- cos(rad(data->angleY))
		* (data->map.height * data->distance * data->scale / 2 / 2));
}
...
static int	get_offx(t_data *data)
{
	return (data->offx + WIDTH / 2
		- cos(rad(data->angleY))
		* (data->map.width * data->distance * data->scale / 2)
		+ sin(rad(data->angleY))
		* (data->map.height * data->distance * data->scale / 2));
}
	...
	while (++y < data->map.height)
	{
		...
		while (++x < data->map.width)
		{
			if (y + 1 != data->map.height)
				render_line(data,
					get_pos(data, data->map.points[y][x], data->distance * data->scale),
					get_pos(data, data->map.points[y + 1][x], data->distance * data->scale),
					get_gradient(data->map.points[y][x].color,
						data->map.points[y + 1][x].color));
			if (x + 1 != data->map.width)
				render_line(data,
					get_pos(data, data->map.points[y][x], data->distance * data->scale),
					get_pos(data, data->map.points[y][x + 1], data->distance * data->scale),
					get_gradient(data->map.points[y][x].color,
						data->map.points[y][x + 1].color));
			if ((x == data->map.width - 1) && (y == data->map.height - 1))
			{
				pos = get_pos(data, data->map.points[y][x], data->distance * data->scale);
